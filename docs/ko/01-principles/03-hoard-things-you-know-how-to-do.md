...
이러한 질문에 대한 답에 확신을 가지는 가장 좋은 방법은 _실행되는 코드_로 설명된 것을 보는 것입니다. 무언가가 이론적으로 가능하다는 것을 아는 것과 실제로 해본 것을 직접 본 것은 다릅니다. 소프트웨어 전문가로서 개발해야 할 핵심 자산은 이런 질문들에 대한 방대한 답변 모음이며, 그 답변에 대한 증명도 함께 있어야 합니다.
...
[tools.simonwillison.net](https://tools.simonwillison.net)은 LLM으로 만든 도구와 프로토타입 중 가장 큰 모음입니다. 저는 이곳을 이용해 제가 [HTML 도구](https://simonwillison.net/2025/Dec/10/html-tools/)라고 부르는 것들을 수집합니다. 이는 JavaScript와 CSS를 내장하고 특정 문제를 해결하는 단일 HTML 페이지입니다.
...
이 모든 것을 왜 모을까요? 자신의 능력을 구축하고 확장하는 데 도움이 되는 것 외에도, 그 과정에서 생성하는 자산들은 코딩 에이전트에 대한 강력한 입력이 됩니다.

제가 가장 좋아하는 프롬프팅 패턴 중 하나는 에이전트에게 두 개 이상의 기존 동작 예제를 조합해서 새로운 것을 만들라고 지시하는 것입니다.

이것이 얼마나 효과적인지 구체화하는 데 도움이 된 프로젝트는 제 도구 컬렉션에 처음 추가한 브라우저 기반 [OCR 도구](https://tools.simonwillison.net/ocr)였으며, [여기서 더 자세히 설명했습니다](https://simonwillison.net/2024/Mar/30/ocr-pdfs-images/).
...
결국 최종 결과를 얻기 위해 [몇 번 반복해서 작업](https://gist.github.com/simonw/6a9f077bf8db616e44893a24ae1d36eb)해야 했지만, 그 이후로 제가 계속 혜택을 받고 있는 진정으로 유용한 도구를 만드는 데는 단 몇 분밖에 걸리지 않았습니다.

## 코딩 에이전트가 이를 더 강력하게 만듭니다
...
(여기서 `curl`을 지정한 이유는 Claude Code가 기본적으로 원시 HTML을 반환하는 대신 페이지 내용을 요약하는 WebFetch 도구를 사용하기 때문입니다.)
